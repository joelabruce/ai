...
use ai::{nn::{activations::activation::RELU, layers::convolution2d::{Convolution2dDeprecated, Dimensions}, neural::{NeuralNetwork, NeuralNetworkNode}, trainer::{train_network, TrainingHyperParameters}}, timed};
...
pub fn handwritten_digits(load_from_file: bool, include_batch_output: bool) {
    let time_to_run = timed::timed(|| {
        ...
        let mut nn = NeuralNetwork::new();// Vec<NeuralNetworkNode> = Vec::new();
        nn.add_node(NeuralNetworkNode::Convolution2dLayer(convo1));
        nn.add_node(NeuralNetworkNode::ActivationFunction(RELU));
        nn.add_node(NeuralNetworkNode::MaxPoolLayer(maxpool1));
        nn.add_node(NeuralNetworkNode::Convolution2dLayer(convo2));
        nn.add_node(NeuralNetworkNode::ActivationFunction(RELU));
        nn.add_node(NeuralNetworkNode::MaxPoolLayer(maxpool2));
