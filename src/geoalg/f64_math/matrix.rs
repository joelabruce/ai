...
impl ZMatrix {
    ...
    pub fn elementwise_multiply(&self, rhs: &Self) -> Self {
    ...
    pub fn elementwise_multiply_threaded(&self, rhs: &Self) -> Self {
        assert_eq!(self.columns, rhs.columns, "Columns must match for elementwise multiply.");
        assert_eq!(self.rows, rhs.rows, "Rows must match for elementwise multiply.");
    ...
    pub fn mul_element_wise_partitioned(&self, rhs: &Self, partitioner: &Partitioner) -> Self {
    ...
    pub fn mul_with_transposed(&self, rhs: &Self) -> Self {
    ...
    pub fn mul_with_transposed_partitioned(&self, rhs: &Self, partitioner: &Partitioner) -> Self {
    ...
    pub fn add_row_vector(&self, rhs: &Self) -> Self {
    ...
    /// TODO: write in new _Matrix.
    pub fn add_row_partitioned(&self, rhs: &Self, partitioner: &Partitioner) -> Self {
        assert_eq!(rhs.rows, 1, "Rhs matrix must have 1 row.");
        assert_eq!(self.columns, rhs.columns, "Lhs and rhs must have equal number of columns.");
...
mod tests {
    ...
    #[test]
    fn test_from_vec() {
        let actual = ZMatrix::from_vec(vec![
            1., 2., 3.,
            4., 5., 6.
        ], 2, 3);

        let expected = ZMatrix {
            rows: 2,
            columns: 3,
            values: vec![
                1., 2., 3.,
    ...
    fn identity_1() {
        ...
        let actual = ZMatrix::new_identity(1);
        let expected = ZMatrix{
            rows: 1,
            columns: 1,
            values: vec![1.0f64],
    ...
    fn identity_2() {
        ...
        let actual = ZMatrix::new_identity(2);
        let expected = ZMatrix {
            rows: 2,
            columns: 2,
            values: vec![
    ...
    fn identity_3() {
        ...
        let actual = ZMatrix::new_identity(3);
        let expected = ZMatrix {
            rows: 3,
            columns: 3,
            values: vec![
    ...
    fn test_random_matrix() {
        let m28x28 = ZMatrix::new_randomized_z(28, 28);
    ...
    fn matrix_mul() {
        ...
        let actual = ZMatrix::mul(&lhs, &rhs);
    ...
    fn test_threaded_mul_equals_regular_mul() {
        let lhs = ZMatrix::new_randomized_z(96, 784);
        let rhs = ZMatrix::new_randomized_z(128, 784);
    ...
    fn test_element_wise_multiply_threaded() {
        let lhs = ZMatrix::new_randomized_z(1000, 3001);
        let rhs = ZMatrix::new_randomized_z(1000, 3001);
    ...
    fn test_add_row_partitioned() {
        let lhs = ZMatrix::new_randomized_z(500, 600);
        let rhs = &ZMatrix::new_randomized_z(1, 600);
